...
Localization 과정 
- motion model: 이전 위치 + 이동 명령(odometry)으로 현재 위치를 **예측**
- sensor model: 실제 센서 값(LiDAR, 카메라 등)으로 예측한 위치를 **보정**

**칼만필터**: 선형 시스템에서 예측-보정을 반복하며 위치 추정. Gaussian 분포 가정.
**파티클 필터**: 수많은 후보 위치(파티클)를 뿌려두고, 센서 값과 맞는 파티클에 가중치를 줘서 위치 추정. 비선형 시스템에도 적용 가능.